...
I am a software engineer and architect with over 25 years of experience working for all manner of organisations, from working as a single freelancer, through a number of startups and consultancies, to large companies including [Sun Microsystems](http://sun.com), [The Economist](http://economist.com) and [WPP](https://wpp.com). I am passionate about open source and past contributions can be found below the employment history below. As I started my career in the 90s I have a solid foundation in all the things we used to do by hand (SQL, server admin, C, perl, you know the score).
...
Now heavily into Claude Code and building my AI dotfiles [here](https://github.com/buddhamagnet/dotclaude).
...
WPP is an AI-first company and developer workflow has transitioned from Devin to Claude Code with engineers contributing skills, hooks and plugins to an internal marketplace. My own Claude dotfiles are growing at [https://github.com/buddhamagnet/dotclaude](https://github.com/buddhamagnet/dotclaude), with my global config synchronised on every Claude invocation. Heavy use of Claude remote (teleport) via the GH app. Work on China project involved deployment of a wide range of China-only LLMs along with monitoring performance and cost analysis (Langfuse) and working on an agentic orchestration that allows users to create, configure and connect agents to tools via the Creative Studio offering. Heavy collaboration with the China team inspired me to learn Mandarin, with a Claude hook that teaches me a new word on every task stop. I see AI as a huge accelerator but maintain my own skills alongside the enormous boost it provides. 

## SKILLSET

- **ML**: From recent work at WPP, fully immersed in the ML space, on MCP servers, AWS (Bedrock, Claude), GCP (Gemini) and Azure (ChatGPT) as well as a set of other LLMs (Deepseek R1, Imagen, Bria, Dalle, Veo, Amazon Nova Reel). As I worked on this space I also immersed myself in ML tooling to get a deeper sense of how things are put together ([Pandas](https://pandas.pydata.org/), [Numpy](https://numpy.org/), [Matplotlib](https://matplotlib.org/), [SKLearn](https://scikit-learn.org/), [TensorFlow](https://www.tensorflow.org/), [Hex](https://hex.tech/), [Snowflake](https://www.snowflake.com/en/uk/) and [Streamlit](https://streamlit.io/)). ML-assisted development using Windsurf as well as a bunch of other tools including Claude Code. Explored local inference with [Ollama](https://ollama.com/).
...
- **Rust**: deep into Rust as we use the language for Cloudflare workers at WPP and can see many other areas where it could be applied, both at work and in my future work.
...
- **Python**: fully conversant with Python 3, mainly from supporting a fleet of Lambdas at The Economist and through interaction with the data science team. Also assisted with integration of a third party Python team at WPP tasked with building MCP servers for ML. Also fully familiar with [Jupyter Notebooks](https://jupyter.org/try-jupyter/notebooks/?path=notebooks/Intro.ipynb) and [Hex](https://hex.tech/).
...
### MAR 2025 - PRESENT: TECH LEAD - WPP - CHINA PROJECT

Joined WPP as a tech lead working on ML projects across 15 teams. Initially working on the platform team supporting 100+ engineers across the platform. Tech side specialises in ML and AI on AWS (Bedrock, Claude), GCP (Gemini) and Azure (ChatGPT) as well as a set of other LLMs (Deepseek R1, Imagen, Bria, Dalle, Veo, Amazon Nova Reel) integrating with ML agents to create ML-driven products for a large portfolio of clients and businesses. Work on platform involved extensive work on CICD (GH actions) and developer experience.

After 4 weeks on the platform team, seconded to a new project to assist with some technical challenges (team lacked AWS experience), namely building out WPP Open Pro, specifically the video generation engine. Instrumental in the success of this project see the [WPP article](https://www.wpp.com/en/news/2025/10/wpp-unveils-wpp-open-pro).

Now working on the platform team again, both on the main team and as engineering lead on a project to ship the Creative Studio suite to AWS China. Involved a rebuild of the auth layer (as China does not support Cognito user pools which are heavily used by the current system), adjusting CICD to deploy to China (using an OIDC provider to deploy cross-partition) and working with the Beijing team to address limitations and compliance challenges along the way. A host of technical and cultural challenges including service and network limitations in China (the Great Firewall), comms with Chinese back end and prompt engineers, testing Hanzi responses from Chinese LLMs and more.
